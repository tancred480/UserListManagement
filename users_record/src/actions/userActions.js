...
const user_entry=(name,email,phoneno,address)=>async(dispatch)=>{
    dispatch({
        type:USER_ENTRY_REQUEST,
        payload:{name,email,phoneno,address}
    });
    try {
        const {data} = await Axios.post("/api/user/user_entry",{name,email,phoneno,address});
        dispatch({
